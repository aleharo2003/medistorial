...
{
    public abstract class RepoBase<T> : IDisposable, IRepository<T> where T : EntityBase, new()
    {
        protected readonly MedistorialDbContext Db;
        protected RepoBase()
        {
            Db = new MedistorialDbContext();
            Table = Db.Set<T>();
        }
        protected RepoBase(DbContextOptions<MedistorialDbContext> options)
        {
            Db = new MedistorialDbContext(options);
            Table = Db.Set<T>();
        }

        protected DbSet<T> Table;
        public MedistorialDbContext Context => Db;
